...
function::MaskTemplate::MaskTemplate(std::string &template_string) {
    ...
    if(check_skip_symbol(template_string, type)) return;
    ...
    if(check_non_empty_list(template_string, type)) return;
    if(check_tuple(template_string, type)) return;
    if(check_complex_value(template_string, type)) return;
...
function::MaskTemplate::MaskTemplate(const PartiallyParsedString &template_string) {

}


function::ComplexMaskTemplate::ComplexMaskTemplate(std::string &template_string) : MaskTemplate(template_string) {
